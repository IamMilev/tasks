...
for (const [listKey, listValue] of Object.entries(markets)) {
  for (const [marketKey, marketValue] of Object.entries(marketCategories.rules)) {
    if (listKey == marketValue.key) {
      obj = { [listKey]: listValue };
      Object.assign(result, obj);
    }
  }
}

const objToArr = Object.keys(result).map((keyProp) => ({
  [keyProp]: result[keyProp],
}));

objToArr.sort((a, b) => {
  const firstV = Object.values(a)[0].info.order;
  const secondV = Object.values(b)[0].info.order;
