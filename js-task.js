...
const markets = {
  ...
  for (const [listKey, listValue] of Object.entries(markets)) {
    for (const [marketKey, marketValue] of Object.entries(marketCategories.rules)) {
      if (listKey == marketValue.key) {
        let obj = {[listKey]: listValue};
        Object.assign(result, obj);
      }
    }
  }
  
  const numArray = Object.keys(result).map(keyProp => ({[keyProp]: result[keyProp]}));
  
  numArray.sort((a, b) => {
    const aVal = Object.values(a)[0].info.order;
    const bVal = Object.values(b)[0].info.order;
    return aVal - bVal;
  })
  
  for (let i = 0; i < numArray.length; i++) {
    Object.assign(sortedResult, numArray[i]);
  }
  ...
  // let sortedResult = Object.values(result).sort((a,b)=> (a.info.order > b.info.order) ? 1 : -1);
  // console.log(sortedResult);
